...
"""
    ...
    async def test_set_gamescope_green_tint_fix_enabled_writes_corrected_full_profile_when_builtin_profile_exists(self):
        with tempfile.TemporaryDirectory() as temp_home, tempfile.TemporaryDirectory() as temp_plugin_dir, tempfile.TemporaryDirectory() as temp_root:
            ...
            disabled_settings = await service.set_gamescope_green_tint_fix_enabled(False)
    ...
    async def test_set_gamescope_green_tint_fix_enabled_rewrites_same_file_between_variants(self):
        with tempfile.TemporaryDirectory() as temp_home, tempfile.TemporaryDirectory() as temp_plugin_dir:
            _write_gamescope_display_assets(Path(temp_plugin_dir))
            helper = main.gamescope_display_profiles.GamescopeDisplayProfiles(
                user_home=temp_home,
            ...
            managed_base_path = managed_dir / "zotac.zone.oled.lua"

            await service.set_gamescope_zotac_profile_enabled(True)
            green_settings = await service.set_gamescope_green_tint_fix_enabled(True)

            self.assertTrue(managed_base_path.exists())
            self.assertEqual(managed_base_path.read_text(encoding="utf-8"), _DECKYZONE_GREEN_TINT_PROFILE)
            ...
            base_settings = await service.set_gamescope_green_tint_fix_enabled(False)
    ...
    async def test_disabling_managed_gamescope_zotac_profile_removes_single_managed_file_and_clears_green_state(self):
        with tempfile.TemporaryDirectory() as temp_home, tempfile.TemporaryDirectory() as temp_plugin_dir:
            _write_gamescope_display_assets(Path(temp_plugin_dir))
            helper = main.gamescope_display_profiles.GamescopeDisplayProfiles(
                user_home=temp_home,
    ...
    async def test_get_settings_reports_unexpected_managed_profile_content(self):
        with tempfile.TemporaryDirectory() as temp_home, tempfile.TemporaryDirectory() as temp_plugin_dir:
            _write_gamescope_display_assets(Path(temp_plugin_dir))
            managed_base_path = (
                Path(temp_home)
            ...
            managed_base_path.parent.mkdir(parents=True, exist_ok=True)
            managed_base_path.write_text("-- unexpected", encoding="utf-8")
    ...
    async def test_get_settings_migrates_legacy_deckyzone_paths_to_single_managed_file(self):
        with tempfile.TemporaryDirectory() as temp_home, tempfile.TemporaryDirectory() as temp_plugin_dir:
            _write_gamescope_display_assets(Path(temp_plugin_dir))
            legacy_green_path = (
                Path(temp_home)
